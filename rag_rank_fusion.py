...
def rewrite_query_variants(query):
    return [
        query,
        f"Explain: {query}",
        f"What is meant by: {query}",
        f"Overview of: {query}",
    ]
...
def rrf_merge_ranked_lists(results_lists, k=60):
    ...
    # Rebuild doc objects from keys (original content)
...
def query_loop(retriever):
    while True:
        ...
        query_variants = rewrite_query_variants(query)
        search_results = rrf_parallel_retrieval(retriever, query_variants)
